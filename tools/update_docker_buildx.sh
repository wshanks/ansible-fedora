...
buildx_version="$(curl -L https://api.github.com/repos/docker/buildx/releases | python -c "$PYCODE")"

sed -i "s/^docker_buildx_version\:.*/docker_buildx_version: '$buildx_version'/" "$defaults"
echo "Docker buildx version is ${buildx_version}"
